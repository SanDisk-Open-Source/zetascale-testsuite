...
sub test_run {
    ...
    foreach my $p(@prop){
        ...
        for(0 .. $ncntr-1){
           $ret=OpenContainer($node->conn(0), "rename_b_ctr-$_","ZS_CTNR_RW_MODE",$size,$$p[0],$$p[1],$$p[2]);
        }

        for(0 .. $ncntr-1){
                $ret = ZSRename($node->conn(0), $cguids[$_], "rename_c_ctr-$_");
                like ($ret, qr/^OK.*/, $ret);
                $ret = ZSRename($node->conn(0), $cguids[$_], "rename_c_ctr-$_");
        ...
        for(0 .. $ncntr-1){
            $ret=OpenContainer($node->conn(0), "rename_c_ctr-$_","ZS_CTNR_RW_MODE",$size,$$p[0],$$p[1],$$p[2]);
            like($ret, qr/^OK.*/, $ret);
            $ret = ZSRename($node->conn(0), $cguids[$_], "rename_d_ctr-$_");
            like($ret, qr/^OK.*/, $ret);
        }
