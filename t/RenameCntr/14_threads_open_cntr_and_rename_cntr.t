...
sub worker_opencntr {
    my ($con, $cguid, $size, @prop) = @_;
    my $ret;
    foreach my $p(@prop){
        for(0 .. $ncntr-1){
            $ret = ZSOpen($node->conn(0), "ctr-$_","ZS_CTNR_RW_MODE",$size,$$p[0],$$p[1],$$p[2]);
    	}
    }
}
...
sub test_run {
    ...
    foreach my $p(@prop){
        ...
        {    
            ...
            push(@threads, threads->new (\&worker_opencntr, $node->conn(2*$_+1), $cguids[$_], $size, @prop));
        }
        $_->join for (@threads);
        ...
        @cguids = GetContainers($node->conn(0), $ncntr);
