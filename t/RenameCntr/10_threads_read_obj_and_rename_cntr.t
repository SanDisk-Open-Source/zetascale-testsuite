...
sub worker_write {
    ...
    my $keyoff = 1000;
    foreach my $d(@data){
        ReadObjects($con, $cguid, $keyoff, $$d[0], $keyoff, $$d[1], $$d[2]);
    ...
    foreach my $p(@prop){
        ...
        foreach(0..$loop-1){
            ...
            {           
                ...
                push(@threads, threads->new (\&worker_write, $node->conn(2*$_+1), $cguids[$_]));
            }
        }
        $_->join for (@threads);
        ...
        for(0 .. $ncntr-1){
            $ret=OpenContainer($node->conn(0), "rename_19-ctr-$_","ZS_CTNR_RW_MODE",$size,$$p[0],$$p[1],$$p[2]);
