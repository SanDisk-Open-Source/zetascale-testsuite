...
sub test_run {
    ...
    foreach my $p(@prop){
        ...
        for(my $k=1; $k<=64000; $k++){
        	$ret=OpenContainer($node->conn(0), "c\_$k","ZS_CTNR_RW_MODE",0,$$p[0],$$p[1],$$p[2]);
        	$cguid = $1 if($ret =~ /OK cguid=(\d+)/);
            foreach my $d(@data){
            ReadObjects($node->conn(0),$cguid,0,$$d[0],100,$$d[1],$$d[2]);
            }
            CloseContainer($node->conn(0),$cguid);
