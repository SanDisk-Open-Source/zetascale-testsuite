...
sub test_ZSWriteObject_update{
    ...
    chomp($ret);
    like($ret, qr/SERVER_ERROR.*/, "ZSTransactionRollback: Update_ops > 100K, Rollback will fail, expect return $ret");
}
...
sub test_run {
    ...
    for(0 .. $nthread - 1)
    {
        ...
        like($ret, qr/OK.*/, "ZSReadObject: check update $update objects succeed on cguid=$cguid");              
    }
    ...
    for(0 .. $ncntr - 1)
    {
        ...
        like($ret, qr/SERVER_ERROR.*/, "ZSReadObject: check $nobject objects succeed on cguid=$cguid");
    }
