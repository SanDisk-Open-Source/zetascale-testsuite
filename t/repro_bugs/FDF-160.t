...
sub worker_write{
    my ($con,$cguid) = @_;
    my $res;
    $res = ZSSet($node->conn($con), $cguid, 50, 50, 1024, 10000, "ZS_WRITE_MUST_NOT_EXIST");
    like ($res, qr/^OK.*/, $res);
}
...
sub worker_read{
    my ($con,$cguid) = @_;
    my $res;
    $res = ZSGet($node->conn($con), $cguid, 50, 50, 1024, 10000);
    like ($res, qr/^OK.*/, $res);
}
...
sub test_zsck {
    ...
    my $log_dir = "r/repro_bugs/FDF-160/"."$log_name";
    $cmd = "$zsck_bin  $cmd &> $log_dir";
    #print $cmd, "\n";
    system("$load_env $cmd ");
    my $res_zsck = readpipe('grep \''."$grep_content".'\''." $log_dir");
    print "zsck log save to $log_dir\n";
    chomp($res_zsck);
    return $res_zsck;
}
...
sub test_run {
    ...
    my $zsck_result;
    $zsck_result = test_zsck("--btree", "zsck_btree.log", 'btree check succeeded');
    is($zsck_result, 'btree check succeeded'," btree check,".' btree check succeeded');

    $zsck_result = test_zsck("", "zsck_no_btree.log", 'meta check succeeded');
    is($zsck_result, 'meta check succeeded', " no btree check,".' meta check succeeded');
