...
sub test_ZSOverwriteObject{
    my ($conn, $cguid, $key_offset, $key_len, $val_offset, $val_len, $nops, $nsegment) = @_;
    ...
    for(0 .. $nsegment)
    {
        ...
        like($ret, qr/OK.*/, "ZSOverwriteObject: overwrite $nops objects on cguid=$cguid, the range of key_offset from $start_key_offset to $end_key_offset.");
	}
}
...
sub test_run_pure_set {
    ...
    my $overwrite_ops = int(32 * 1024 * 0.5);

    my $overwrite_thread = threads->create(\&test_ZSOverwriteObject,
            $node->conn(1),
            $cguid,
            $key_offset,
            $key_len,
