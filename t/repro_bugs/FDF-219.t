...
sub worker_write{
        ...
        my $ret;
        foreach(1..1500){
            $ret = ZSSet($node->conn($con), $cguid, $_, 100, 128000,1, "ZS_WRITE_MUST_NOT_EXIST");
            like ($ret, qr/^OK.*/, $ret);
            $ret = ZSSet($node->conn($con), $cguid, $_, 101, 64000,1, "ZS_WRITE_MUST_NOT_EXIST");
            like ($ret, qr/^OK.*/, $ret);
...
sub worker_read{
        ...
        foreach(1..$count){
            $ret = ZSGet($node->conn($con), $cguid, $_, 100, 128000, 1);
            like ($ret, qr/^OK.*/, $ret);
            $ret = ZSGet($node->conn($con), $cguid, $_, 101, 64000, 1);
            like ($ret, qr/^OK.*/, $ret);
...
#
{
    ...
    #$node->reboot();
    $node->stop();
    my $zsck_result = test_zsck("--btree", "BTREE"."_zsck_btree.log", 'btree check succeeded');
    print $zsck_result;
    test_run_recovery();
    test_clean();
}
