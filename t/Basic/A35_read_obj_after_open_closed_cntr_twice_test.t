...
sub test_run {
    my $ret;
    my $cguid;
    my @prop=([4,"ZS_DURABILITY_HW_CRASH_SAFE","no"],[4,"ZS_DURABILITY_HW_CRASH_SAFE","no"]);
    #my @data=([50,64000,6250],[100,128000,6250],[150,514,37500]);
    ...
    foreach my $p(@prop){
        ...
        $cguid = $1 if($ret =~ /OK cguid=(\d+)/);
        foreach my $d(@data){
            ReadObjects($node->conn(0),$cguid,1000,$$d[0],1000,$$d[1],$$d[2]);
