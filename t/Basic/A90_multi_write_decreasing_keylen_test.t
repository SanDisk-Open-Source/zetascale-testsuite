...
sub test_run {
    ...
    foreach my $ctr(0..$nctr-1) 
    {
	    ...
	    foreach(0..10)
	    {
	    	push(@threads,threads->new(\&worker_write,11*$ctr+$_,$cguid,$keyoff,$keylen));
    ...
    foreach my $ctr(0..$nctr-1)
    {
	    ...
	    foreach(0..10)
	    {
	    	push(@threads,threads->new(\&worker_read,11*$ctr+$_,$cguid,$keyoff,$keylen));
	       #$keyoff = $keyoff+($num*8000+1);
		$keylen = $keylen - 50;
	    }
    }
    ...
    foreach my $ctr(0..$nctr-1)
    {
	    ...
	    foreach(0..10)
	    {
	    	push(@threads,threads->new(\&worker_delete,11*$ctr+$_,$cguid,$keyoff,$keylen));
	    	#$keyoff = $keyoff+($num*8000+1);
		$keylen = $keylen - 50;
	    }
    } 
