...
sub test_run {
    ...
    foreach my $p(@prop){
        ...
        print "mode is $mode \n";
        if ($mode =~ /.*mode=1.*/){
            like ($ret, qr/SERVER_ERROR ZS_FLASH_ENOSPC.*/, "ZSWriteObject-->cguid=$cguid nops=$nops SERVER_ERROR ZS_FLASH_ENOSPC");
            #like ($ret, qr/SERVER_ERROR ZS_OUT_OF_STORAGE_SPACE.*/, "ZSWriteObject-->cguid=$cguid nops=$nops SERVER_ERROR ZS_OUT_OF_STORAGE_SPACE");
        }elsif ($mode =~ /.*mode=2.*/){
            like ($ret, qr/SERVER_ERROR ZS_FLASH_ENOSPC.*/, "ZSWriteObject-->cguid=$cguid nops=$nops SERVER_ERROR ZS_FLASH_ENOSPC");
        }
        ...
        like ($ret, qr/SERVER_ERROR.*/, "ZSReadObject->cguid=$cguid nops=$nops SERVER_ERROR");
