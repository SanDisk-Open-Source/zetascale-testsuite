...
# file: t/ZS_RangeQuery/31_multi_cntr_5update_1range_rangequery.t
# author: shujing zhu
# email: shujingzhu@hengtiansoft.com
# date: June 18, 2013
# description:  5 cntr0~4,start 10 thread, one thread range query cntr,5 threads update cntrs
...
sub test_RangeQuery{
    ...
    like($ret, qr/OK.*/,"ZSGetRange:Get $key ~ $nobject+$key,cguid=$cguid -> $ret");
    ...
    like($ret, qr/OK n_out=$nobject.*/, "ZSGetNextRange:cguid=$cguid,Get $n_out objects->,$ret");

    $ret = ZSGetRangeFinish($conn);
    like($ret, qr/OK.*/, "ZSGetRangeFinish:cguid=$cguid");
}
    ...
    foreach my $p(@prop){
        for(my $i = 0;$i< $ncntr; $i++){
            ...
            foreach my $d(@data){
                ...
                like($ret, qr/OK.*/, "ZSWriteObject: load $$d[2] objects to cntr$i, cguid=$cguid");
            }
        }
        ...
        foreach my $d(@data){
            ...
            $update_thread[$nthread - 1] = threads->create(\&test_RangeQuery,
                        $node->conn($nthread - 1),
                        $cguid[1],
                        $key,
                        $$d[0],
            ...
            like($ret, qr/OK.*/,"ZSGetRange:Get $key ~ $$d[2]+$key,cntr3,cguid= $cguid[3],flags=$flags");
            ...
            my $n_out = $1 if($ret =~ /OK n_out=(\d+)/);
            like($ret, qr/OK n_out=$$d[2].*/, "ZSGetNextRange:Get $n_out objects,,cguid= $cguid[3] ,$ret");
