...
#
# file: t/ZS_RangeQuery/11_END_flags_invalid_rangequery.t
# author: shujing zhu
...
sub test_run {
    ...
    foreach my $p(@prop){
        ...
        foreach my $d(@data){
            ...
            my $end_key = $$d[2]+$key;
            #bug :end_key default is 0.
            # if have end_key,flags must have one END,and converse as well

            # GE,GT,(or LT,LE) must only have one.
            
            $flags = 'ZS_RANGE_START_GE|ZS_RANGE_END_GT';
            $ret = ZSGetRange (
                        $node->conn(0),
                        cguid         => $cguid,
                        keybuf_size   => 60, 
            ...
            $flags = 'ZS_RANGE_SATRT_LT|ZS_RANGE_END_LE';
            $ret = ZSGetRange (
                        $node->conn(0),
                        cguid         => $cguid,
                        keybuf_size   => 60,
            ...
            $flags = 'ZS_RANGE_START_LE|ZS_RANGE_END_LT';
            $ret = ZSGetRange (
                        $node->conn(0),
                        cguid         => $cguid,
                        keybuf_size   => 60,
            ...
            #END flags must have only one.
            $flags = 'ZS_RANGE_END_GT|ZS_RANGE_END_LE';
            $ret = ZSGetRange (
                        $node->conn(0),
                        cguid         => $cguid,
                        keybuf_size   => 60,
            ...
            $flags = 'ZS_RANGE_END_GT|ZS_RANGE_END_GE|ZS_RANGE_START_LT';
            $ret = ZSGetRange (
                        $node->conn(0),
                        cguid         => $cguid,
                        keybuf_size   => 60,
