...
# file: t/ZS_RangeQuery/26_single_cntr_10Insert_1range_rangequery.t
# author: shujing zhu
...
sub test_ZSWriteObject_create{
	...
	like($ret, qr/OK.*/, "ZSWriteObject, load $nops objects on cguid=$cguid");
}
...
sub test_run {
    ...
    foreach my $p(@prop){
        ...
        foreach my $d(@data){
            ...
            my $object_per_thread = $re_nobject / ($nthread-1);
            for(0 .. $nthread - 2)
            {
                $update_thread[$_] = threads->create(\&test_ZSWriteObject_create, 
                        $node->conn($_), 
                        $cguid, 
                        $$d[2] - $re_nobject + $key+$_ * $object_per_thread, 
                        $$d[0], 
                        $$d[2] - $re_nobject + $key+$_ * $object_per_thread, 
                        $$d[1] , 
                        $re_nobject / ($nthread-1));
            }
            ...
            like($ret, qr/OK.*/, "ZSReadObject: check $$d[2] objects succeed on $cname, cguid=$cguid ->$ret");
        ...
        foreach my $d(@data){
            ...
            like($ret, qr/OK.*/, "ZSReadObject: check $$d[2] objects succeed after restart ZS on $cname, cguid=$cguid ->$ret");
        }
        $ret = ZSCloseContainer(
                    $node->conn(0),
                    cguid      => $cguid,
