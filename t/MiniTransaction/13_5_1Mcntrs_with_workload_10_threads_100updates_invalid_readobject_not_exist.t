...
# file: tt/MiniTransaction/13_5_1Mcntrs_with_workload_10_threads_100updates_invalid_readobject_not_exist.t
...
sub test_ZSWriteObject_update{
    ...
    foreach my $d(@data){
        ...
        like($ret, qr/OK.*/, "ZSWriteObject, update ".$nops*$$d[2]." objects on Cntr_cguid=$cguid");
    }

#Read a non exist object
    $ret = ZSReadObject(
                    $node->conn($connid),
                    cguid         => $cguid,
    ...
    like($ret, qr/SERVER_ERROR 1 items read failed 0 items check failed/, "ZSReadObject, read an non_existent obect, expect return SERVER_ERROR 1 items read failed 0 items check failed");
