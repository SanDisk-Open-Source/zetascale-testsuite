...
#
# file: t/MiniTransaction/05_5_1Mcntrs_5_threads_transcommit_without_transstart.t
...
sub test_ZSWriteObject_update{
    ...
    if ($mode =~ /.*mode=1.*/)
    {
        like($ret, qr/SERVER_ERROR ZS_FAILURE_OPERATION_DISALLOWE/, 'ZSTransactionCommit without start,btree type container  expect return ZS_FAILURE_OPERATION_DISALLOWE');
    }
    elsif ($mode =~ /.*mode=2.*/)
    {
        like($ret, qr/SERVER_ERROR ZS_FAILURE_NO_TRANS.*/, "ZSTransactionCommit without start, hash type container expect return ZS_FAILURE_NO_TRANS");
    }
}
