...
sub worker{
    ...
    if ($mode =~ /.*mode=1.*/){
        like($ret, qr/SERVER_ERROR ZS_UNSUPPORTED_REQUEST.*/, 'ZSTransactionRollback again, expect return SERVER_ERROR ZS_UNSUPPORTED_REQUEST.');
    }elsif ($mode =~ /.*mode=2.*/){
        like($ret, qr/SERVER_ERROR ZS_FAILURE.*/, 'ZSTransactionRollback again, expect return SERVER_ERROR ZS_FAILURE');
    }
}
        ...
        for my $i(0 .. $ncntr - 1){
            for my $j(0 .. $nthread - 1){
                if ($mode =~ /.*mode=1.*/){
                    ...
                    push(@threads, threads->new(\&read, $node->conn($i*$nthread+$j),$j,$cguids[$i],$keyoff,$valoff,$up_per_th));
                }
            }
        }
        $_->join for (@threads);
