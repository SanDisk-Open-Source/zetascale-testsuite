...
sub test_ZSWriteObject_update{
    ...
    {
        like($ret, qr/OK.*/, 'Hash type container:ZSTransactionRollback '.$ret);
    }
    ...
    if ($mode =~ /.*mode=1.*/)
    {
        like ($ret, qr/SERVER_ERROR ZS_UNSUPPORTED_REQUEST.*/, 'Btree type container ZSTransactionRollback again '.$ret);
    }
    elsif ($mode =~ /.*mode=2.*/)
    {
        like($ret, qr/SERVER_ERROR ZS_FAILURE_NO_TRANS.*/, 'Hash type container ZSTransactionRollback again'.$ret);
    }
...
sub test_run {
	...
	for(0 .. $ncntr - 1)
	{
		...
		{
                    like($ret, qr/OK.*/, "Hash type ctr ZSReadObject: check update ".$update_per_cntr*$$d[2]." objects on cguid=$cguid[$_]");
		}
            }
	}
