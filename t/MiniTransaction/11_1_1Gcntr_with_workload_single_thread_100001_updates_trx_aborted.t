...
sub test_run {
	...
	for(0 .. $nthread - 1)
	{
	    ...
	    {
		like($ret, qr/OK.*/, "ZSReadObject: check hash type container data rollback  on cguid=$cguid[$_]");
	    }
	...
	for(0 .. $ncntr - 1)
	{
	    ...
	    push(@cguid, $cguid);
            ...
            if ($mode =~ /.*mode=1.*/)
            {
                ...
                like($ret, qr/OK.*/, "ZSReadObject: check btree type container data update on cguid=$cguid[$_]");
	    }
	    elsif ($mode =~ /.*mode=2.*/)
	    {
                $ret = ZSReadObject(
