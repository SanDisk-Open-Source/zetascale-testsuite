...
sub test_ZSWriteObject_update{
	...
	foreach my $d(@data){
        ...
        if ($mode =~ /.*mode=1.*/)
        {
                like($ret, qr/SERVER_ERROR ZS_FAILURE_OPERATION_DISALLOWE/, 'ZSTransactionCommit again, btree type container expect return ZS_FAILURE_OPERATION_DISALLOWE ');
        }
        elsif ($mode =~ /.*mode=2.*/)
        {
                like($ret, qr/SERVER_ERROR ZS_FAILURE_NO_TRANS.*/, "ZSTransactionCommit again, hash type container expect return ZS_FAILURE_NO_TRANS");
        }
}
