...
sub test_run {
    ...
    @prop = (["yes","yes","no","ZS_DURABILITY_HW_CRASH_SAFE","no"],["yes","yes","no","ZS_DURABILITY_HW_CRASH_SAFE","yes"]);
	...
	for(0 .. $nthread - 1)
	{
		...
		like($ret, qr/OK.*/, "ZSReadObject: check update ".$update*$$d[2]." objects succeed on cguid=$cguid[$_]");              
	    }
	}
	...
	for(0 .. $ncntr - 1)
	{
		...
		like($ret, qr/OK.*/, "ZSReadObject: check update ".$update*$$d[2]." objects succeed on cguid=$cguid");

                $ret = ZSReadObject(
                    $node->conn(0),
                    cguid         => $cguid[$_],
                    key_offset    => $key_offset+$update*$$d[2],
                    key_len       => $$d[0],
                    data_offset   => $val_offset+$update*$$d[2],
                    data_len      => $$d[1],
                    nops          => ($nobject-$update)*$$d[2],
                    check         => "yes",
                    );
                like($ret, qr/OK.*/, "ZSReadObject: check update ".($nobject-$update)*$$d[2]." objects succeed on cguid=$cguid");
	    }
	}
