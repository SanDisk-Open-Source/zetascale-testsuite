...
#
# file: t/MiniTransaction/06_5_1Gcntrs_5_parallel_threads_double_transstart.t
...
sub test_ZSWriteObject_update{
    ...
    #Transaction start again, without Transaction commit
    $ret = ZSTransactionStart(
            $conn,
            );
    if ($mode =~ /.*mode=1.*/)
    {
        like($ret, qr/OK.*/, 'ZSTransactionStart again, Btree type container support embedded trx.');
    }
    elsif ($mode =~ /.*mode=2.*/)
    {
        like($ret, qr/SERVER_ERROR ZS_FAILURE/, 'ZSTransactionStart again,Hash type container not support embedded trx ');
    }
}
