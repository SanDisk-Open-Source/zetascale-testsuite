...
sub test_ZSWriteObject_update{
    ...
    like($ret, qr/OK.*/, "ZSTransactionStart: cguid=$cguid");
    ...
    like($ret, qr//, "ZSTransactionCommit: cguid=$cguid will failed after kill the node.");
...
sub test_run {
	...
	for(0 .. $nthread - 1)
	{
            my $nops = 1000*(int($_/2)+1);
            if($flags[$_]==0){
	        foreach my $d(@data){
                    $ret = ZSReadObject(
                            $node->conn(0),
                            cguid         => $cguid[$_/2],
                            key_offset    => $key_offset + $nops*$$d[2]*($_%2),
                            key_len       => $$d[0] + $_/2,
                            data_offset   => $val_offset + $nops*$$d[2]*($_%2),
                            data_len      => $$d[1],
                            nops          => $nops*$$d[2],
                            check         => "yes",
                            );
                    like($ret, qr/OK.*/, "ZSReadObject: read rollbacked ".$nops*$$d[2]." objects on Tran_Cntr$_, cguid=$cguid[$_]");
	        }
            }elsif($flags[$_]==1){
                foreach my $d(@data){
                    $ret = ZSReadObject(
                            ...
                            key_offset    => $key_offset + $nops*$$d[2]*($_%2),
                            key_len       => $$d[0] + $_/2,
                            data_offset   => $val_offset-1 + $nops*$$d[2]*($_%2),
                            data_len      => $$d[1],
                            nops          => $nops*$$d[2],
                            check         => "yes",
                    );
                    like($ret, qr/OK.*/, "ZSReadObject: read updated ".$nops*$$d[2]." objects on Tran_Cntr$_, cguid=$cguid[$_]");
                }
            }
	}
