...
sub test_zsmetafault_and_zsck {
    ...
    my $zs_log_path="r/Zsck/Z01_metafault_label_recovery_64K/log_container_".$c_id.".log";
    my $zs_metafault_log_path="r/Zsck/Z01_metafault_label_recovery_64K/metafault.log";
    ...
    # zsmetafault test
    my $cmd = "";
    $cmd = $cmd . "$zsmetafault --container=".$c_id." --label 2>".$zs_metafault_log_path;
    print $cmd, "\n";
    system("$load_env $cmd");
    ...
    while(<LOG>)
    {
        if(/mcd_check_label/)
        {
            like($_, qr/mcd_check_label failed.*/, "mcd_check_label failed in meta check.\n");
        }
    }
    ...
    while(<LOG>)
    {
        if(/invalid/)
        {
            like($_, qr/flash label invalid.*/, "flash label invalid in /tmp/zsck.log.\n");
        }
    }
...
#
{
        ...
        test_recovery();
        test_clean();
        copy("./r/Zsck/Z01_metafault_label_recovery_64K/server.log", "./r/Zsck/Z01_metafault_label_recovery_64K/server_".$c_id.".log");
    
        $c_id = 0;
        print "<<<<<< Test with container=".$c_id." >>>.\n";
        test_init();                                                                                                                                                    test_run();
        test_zsmetafault_and_zsck();                                                    test_recovery();
        test_clean();                                                                   copy("./r/Zsck/Z01_metafault_label_recovery_64K/server.log", "./r/Zsck/Z01_metafault_label_recovery_64K/server_".$c_id.".log");
