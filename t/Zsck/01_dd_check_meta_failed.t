...
sub test_zsck {
    ...
    my $log_dir = "r/Zsck/01_dd_check_meta_failed/"."$log_name";
    system("dd if=/dev/urandom of=/schooner/data/schooner0 seek=40000 count=5000");
...
sub test_run {
	...
	my $zsck_result;
	$zsck_result = test_zsck("", "$ctr_type"."_zsck_no_btree.log", 'meta check failed');
	like($zsck_result, qr/^meta check failed.*/, "$ctr_type type container, no btree check,".' meta check failed');
	$zsck_result = test_zsck("--btree", "$ctr_type"."_zsck_btree.log", 'meta check failed');
	like($zsck_result, qr/^meta check failed.*/,"$ctr_type type container, btree check,".' meta check failed');
