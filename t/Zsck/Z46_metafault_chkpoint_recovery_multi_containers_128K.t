...
sub test_zsmetafault_and_zsck {
    ...
    my $zs_log_path="r/Zsck/Z46_metafault_chkpoint_recovery_multi_containers_128K/log_container_".$c_id.".log";
    my $zs_metafault_log_path="r/Zsck/Z46_metafault_chkpoint_recovery_multi_containers_128K/metafault.log";
    ...
    # zsmetafault test
    my $cmd = "";
    $cmd = $cmd . "$zsmetafault --container=".$c_id." --chkpoint 2>".$zs_metafault_log_path;
    print $cmd, "\n";                                                               system("$load_env $cmd");
    ...
    while(<LOG>)
    {
        if(/mcd_check_ckpt_descriptors/)  
        {
            like($_, qr/mcd_check_ckpt_descriptors failed.*/, "mcd_check_ckpt_descriptors failed in meta check.\n");
        }
    }
    ...
    while(<LOG>)
    {
        if(/invalid/) 
        {
            like($_, qr/ZSCHECK_MAGIC_ERROR ckpt descriptor magic number invalid.*/, "ckpt descriptor magic number invalid in /tmp/zsck.log.\n");
        }
    }
...
#
{
    for($c_id=-1;$c_id<=2;$c_id++)
    {
        ...
        test_run();
        test_clean();
        copy("./r/Zsck/Z46_metafault_chkpoint_recovery_multi_containers_128K/server.log", "./r/Zsck/Z46_metafault_chkpoint_recovery_multi_containers_128K/server_".$c_id.".log")
    }
