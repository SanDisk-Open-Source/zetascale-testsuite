...
sub test_zsmetafault_and_zsck {
   ...
   my $zs_log_path="r/Zsck/Z38_metafault_flog_recovery_multi_containers_128K/zsck_container_".$c_id.".log";
    my $zs_metafault_log_path="r/Zsck/Z38_metafault_flog_recovery_multi_containers_128K/metafault.log";
    ...
    # zsmetafault test
    my $cmd = "";
    $cmd = $cmd . "$zsmetafault --container=".$c_id." --flog 2>".$zs_metafault_log_path;
    print $cmd, "\n";
    system("$load_env $cmd");
    ...
    open(LOG, "< $zs_log_path") or die "Cannot open log file $zs_log_path !\n";     while(<LOG>)
    {
        if(/Flog check/)
        {
            like($_, qr/Flog check failed.*/, "Flog check failed in meta check.\n");
        }
    }
    ...
    while(<LOG>)
    {
        if(/invalid/)
        {
            if(/ZSCHECK_MAGIC_ERROR/)
            {
                like($_, qr/ZSCHECK_MAGIC_ERROR flog magic number invalid.*/, "Flog magic number invalid in /tmp/zsck.log.\n");
            }
            else
            {
                like($_, qr/ZSCHECK_CHECKSUM_ERROR flog checksum invalid.*/, "Flog checksum invalid in /tmp/zsck.log.\n");
            }
        }
    }
...
#
{
    for($c_id=-1;$c_id<=2;$c_id++)
    {
        ...
        test_run();
        test_clean();
        copy("./r/Zsck/Z38_metafault_flog_recovery_multi_containers_128K/server.log", "./r/Zsck/Z38_metafault_flog_recovery_multi_containers_128K/server_".$c_id.".log")
    }
