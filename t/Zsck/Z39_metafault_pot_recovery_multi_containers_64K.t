...
sub test_zsmetafault_and_zsck {
    ...
    my $zs_log_path="r/Zsck/Z39_metafault_pot_recovery_multi_containers_64K/log_container_".$c_id.".log";
    my $zs_metafault_log_path="r/Zsck/Z39_metafault_pot_recovery_multi_containers_64K/metafault.log";
    ...
    while(<LOG>) 
    {
        if(/failed/)                                                                    {                                                                                   if(/POT check/)                                                                 {                                                                                    like($_, qr/POT check failed.*/, "POT check failed in meta check.\n");                                                                                     }                                                                               if(/potbm/)                                                                     {                                                                                   like($_, qr/vdc potbm: failed.*/, "vdc potbm check failed in meta check.\n");                                                                               }
        }
    }
...
#
{
    for($c_id=-1;$c_id<=2;$c_id++)
    {
        ...
        test_run();
        test_clean();
        copy("./r/Zsck/Z39_metafault_pot_recovery_multi_containers_64K/server.log", "./r/Zsck/Z39_metafault_pot_recovery_multi_containers_64K/server_".$c_id.".log");
    }
