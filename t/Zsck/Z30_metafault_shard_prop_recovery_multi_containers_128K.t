...
sub test_zsmetafault_and_zsck {                                                     
    my $ret;
    my $load_env = "export ZS_PROPERTY_FILE=$node->{prop};  LD_PRELOAD=$node->{sdk}/lib/libpthread.so.0:/usr/lib64/liblz4.so ZS_LIB=$node->{sdk}/lib/libzs.so"; 
    my $zs_log_path="r/Zsck/Z30_metafault_shard_prop_recovery_multi_containers_128K/log_container_".$c_id.".log"; 
    my $zs_metafault_log_path="r/Zsck/Z30_metafault_shard_prop_recovery_multi_containers_128K/metafault.log";
    ...
    # zsmetafault test
 my $cmd = ""; 
    $cmd = $cmd . "$zsmetafault --container=".$c_id." --shard_prop 2>".$zs_metafault_log_path;                                                                      print $cmd, "\n";
    system("$load_env $cmd");
    ...
    {
        if(/mcd_check_shard_properties/)
        {
        like($_, qr/mcd_check_shard_properties failed.*/, "mcd_check_shard_properties failed in meta check.\n");  
        } 
    }
    ...
    while(<LOG>)
    {
        if(/invalid/)
        { 
           like($_, qr/shard properties magic number invalid.*/, "shard proprerties magic number invalid in /tmp/zsck.log.\n"); 
        }
    } 
    unlink("/tmp/zsck.log"); 
    return;
...
#
{
    for($c_id=-1;$c_id<=2;$c_id++)
    {
        ...
        copy("./r/Zsck/Z30_metafault_shard_prop_recovery_multi_containers_128K/server.log", "./r/Zsck/Z30_metafault_shard_prop_recovery_multi_containers_128K/server_".$c_id.".log")
    }
