...
sub test_zsck {
    ...
    print "enter\n";
    my $log_dir = "r/Zsck/BAS_10_OneModeCtnr_128K_ObjSize_DoubleZsck_NormalShutdown/"."$log_name";
...
sub test_run {
    ...
    if ($ctr_type eq "BTREE"){
        $zsck_result = test_zsck("--btree", "$ctr_type"."_zsck_double_btree.log", 'btree check succeeded');
    is($zsck_result, 'btree check succeeded',"$ctr_type type container, btree check,".' second btree check succeeded');
        $zsck_result = test_zsck("", "$ctr_type"."_zsck_double_no_btree.log", 'meta check succeeded');
    is($zsck_result, 'meta check succeeded', "$ctr_type type container, no btree check,".' second meta check succeeded');
    }
    else{
        $zsck_result = test_zsck("--btree", "$ctr_type"."_zsck_double_btree.log", ' meta check succeeded');
