...
sub test_zsmetafault_and_zsck {
    ...
    my $zs_log_path = "r/Zsck/Z42_metafault_potbitmap_recovery_multi_containers_128KB/log_container_".$c_id.".log";
    my $zs_metafault_log_path = "r/Zsck/Z42_metafault_potbitmap_recovery_multi_containers_128KB/metafault.log";
    ...
    # zsmetafault test
    my $cmd = "";
    $cmd = $cmd . "$zsmetafault --container=".$c_id." --potbitmap 2>".$zs_metafault_log_path;
    print $cmd, "\n";
    system("$load_env $cmd");
    ...
    while(<LOG>)
    {
        if(/POT bitmap check/) 
        {
            like($_, qr/POT bitmap check failed.*/, "POT bitmap check failed in meta check.\n");
        } 
    }                                                                           
    ...
    { 
        if(/invalid/) 
        {
            like($_, qr/pot bitmap magic number invalid.*/, "pot bitmap magic number invalid in /tmp/zsck.log.\n");
        } 
    }
...
#
{
    for($c_id=-1;$c_id<=2;$c_id++)
    {
        ...
        test_run();
        test_clean();
        copy("./r/Zsck/Z42_metafault_potbitmap_recovery_multi_containers_128K/server.log", "./r/Zsck/Z42_metafault_potbitmap_recovery_multi_containers_128K/server_".$c_id.".log")
    }
