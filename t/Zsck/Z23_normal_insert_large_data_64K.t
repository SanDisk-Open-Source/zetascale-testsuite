...
sub test_recovery {
    ...
    like($ret, qr/OK.*/, 'Node recovery');
    subtest "Test with ctnr c0" => sub {
        $ret=OpenContainer($node->conn(0), "c0","ZS_CTNR_RW_MODE",1048576,0,"ZS_DURABILITY_PERIODIC","yes");
        $cguid = $1 if($ret =~ /OK cguid=(\d+)/);
        ReadObjects($node->conn(0),$cguid,1000,50,1000,64000,500);
        ReadObjects($node->conn(0),$cguid,1000,51,1000,512,500);
        ReadObjects($node->conn(0),$cguid,1000,52,1000,512,500);
        ReadObjects($node->conn(0),$cguid,1000,53,1000,512,500);
...
sub test_zsmetafault_and_zsck {
    ...
    my $zs_log_path="r/Zsck/Z23_normal_insert_large_data_64K/log_container_".$c_id.".log";
    my $zs_metafault_log_path="r/Zsck/Z23_normal_insert_large_date_64K/metafault.log";

    my $zsmetafault = "$node->{sdk}/utlis/zsmetafault";
    ...
    while(<LOG>)
    {
        if(/meta check/)
        {
            like($_, qr/meta check succeeded/, "meta check succeeded in meta check.\n");
        }
    }
    ...
    while(<LOG>)
    {
        if(/successful/)
        {
            like($_, qr/Btree check successful.*/, "Btree check in /tmp/zsck.log.\n");
        }
    }
...
#
{
    for($c_id=-1;$c_id<=2;$c_id++)
    {
            ...
            test_recovery();
            test_clean();
            copy("./r/Zsck/Z23_normal_insert_large_data_64K/server.log", "./r/Zsck/Z23_normal_insert_large_data_64K/server_".$c_id.".log");
    }
