...
sub worker_write{
    my ($con,$cguid) = @_;
    my $res;
    print "---The write thread for container $cguid start!---\n";
...
#    like ($res, qr/^OK.*/, $res);
    print "---The write thread for container $cguid stop!---\n";
}

sub worker_read{
    my ($con,$cguid) = @_;
    my $res;
    $res = ZSGet($node->conn($con), $cguid, 50, 50, 1000*128, 50000);
    like ($res, qr/^OK.*/, $res);
...
sub test_zsck {
    ...
    my $log_dir = "r/Zsck/BAS_08_OneModeCtnr_128K_ObjSize_Not_Finish_UnnormalShutdown/"."$log_name";
