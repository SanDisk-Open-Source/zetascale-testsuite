...
sub worker{
    ...
    foreach my $d(@data){
        $ret = DeleteLogObjects($node->conn($connid), $cguid, $$d[0]+$nops*$$d[4]*$j, $$d[1], $$d[2], $valoff+$nops*$$d[4]*$j, $$d[3], $nops*$$d[4], "ZS_WRITE_MUST_EXIST");
        like($ret, qr/.*/, $ret);
    }
    ...
    like($ret, qr/.*/, 'ZSTransactionRollback success');
}
