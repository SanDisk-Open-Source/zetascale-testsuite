...
sub worker{
    ...
    foreach my $d(@data){
        $ret = DeleteLogObjects($node->conn($connid), $cguid, $$d[0]+$nops*$$d[4]*$j, $$d[1], $$d[2], $nops*$$d[4]);
        like($ret, qr/.*/, $ret);
    }
...
sub read{
    my ($conn, $j, $cguid, $valoff, $off, $nops, $flag) = @_;
    my $ret;
...
sub test_run {
    ...
    foreach my $p(@prop){
        ...
        for my $i(0 .. $ncntr - 1){
            for my $j(0 .. $nthread - 1){
                push(@threads, threads->new(\&read, $node->conn($i*$nthread+$j),$j,$cguids[$i],$valoff,0,$up_per_th,$flags[$i*$nthread+$j]));
            }
        }
        $_->join for (@threads);
        ...
        for my $i(0 .. $ncntr - 1){
            for my $j(0 .. $nthread - 1){
                push(@threads, threads->new(\&read, $node->conn($i*$nthread+$j),$j,$cguids[$i],$valoff,$update/$ncntr,$other_per_th,0));
            }
        }
        $_->join for (@threads);
