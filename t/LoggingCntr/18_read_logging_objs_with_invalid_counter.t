...
sub read{
    ...
    foreach my $d(@data){
        ...
        $ret = ReadLogObjects($conn, $cguid, $$d[0]+$nops*$$d[4]*$j+1, $$d[1], $$d[2], $valoff+$nops*$$d[4]*$j, $$d[3], $nops*$$d[4]);
...
sub test_run {
    ...
    foreach my $p(@prop){
        ...
        for(0 .. $ncntr-1){
            foreach my $d(@data){
                $ret = WriteLogObjects($node->conn(0), $cguids[$_], $$d[0], $$d[1], $$d[2], $valoff, $$d[3], $nobject*$$d[4]);
                like($ret, qr/OK.*/, $ret);
            }
        }
