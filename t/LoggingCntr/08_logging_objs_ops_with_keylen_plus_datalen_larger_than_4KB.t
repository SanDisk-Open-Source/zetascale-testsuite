...
my @data = ([50, "aa", "bbb", 4000, 0.25], [100, "cdefm", "zzz", 4998, 0.25], [150, "hh", "osd", 3990, 0.25], [200, "w", "nuyt", 8990, 0.25]);#counter,pg,osd,vallen,nops

#my @data = ([150, "hh", "osd", 3990, 0.25]);
...
sub read{
    ...
    foreach my $d(@data){
        ...
        if($$d[2] eq "osd"){like ($ret, qr/OK.*/, $ret);}
        else {like($ret, qr/SERVER_ERROR.*/, $ret);}
    }
}
...
sub test_run {
    ...
    foreach my $p(@prop){
        ...
        for(0 .. $ncntr-1){
            foreach my $d(@data){
                $ret = WriteLogObjects($node->conn(0), $cguids[$_], $$d[0], $$d[1], $$d[2], $valoff, $$d[3], $nobject*$$d[4], "ZS_WRITE_MUST_NOT_EXIST");
                if($$d[2] eq "osd"){like($ret, qr/OK.*/, $ret);}
                else {like($ret, qr/SERVER_ERROR ZS_OBJECT_TOO_BIG.*/, $ret);}
            }
