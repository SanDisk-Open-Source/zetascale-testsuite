    ...
    foreach my $p(@prop){
        for(1 .. $ncntr)
        {
            ...
            foreach my $d(@data){
                ...
                #enum deleted objs
                $ret = DeleteLogObjects($node->conn(0), $cguid, $$d[0], $$d[1], $$d[2], $$d[5]/2);
                like($ret, qr/OK.*/, $ret);

                $enum_num = $enum_num - $$d[5]/2;
                $ret = EnumeratePG($node->conn(0), $cguid, $$d[0], $$d[1], $$d[2]);
                like($ret, qr/pg=$$d[1] OK.*enumerate $enum_num objects.*/, "$ret");
            }
        ...
        for(1 .. $ncntr)
        {
            ...
            foreach my $d(@data){
                ...
                $ret = ReadLogObjects($node->conn(0), $cguid, $$d[0]+$$d[5]/2, $$d[1], $$d[2], $$d[3]+$$d[5]/2, $$d[4], $enum_num);
                like($ret, qr/OK.*/, $ret);
               
                #enum deleted objs
                $ret = DeleteLogObjects($node->conn(0), $cguid, $$d[0]+$$d[5]/2, $$d[1], $$d[2], $enum_num);
                like($ret, qr/OK.*/, $ret);
