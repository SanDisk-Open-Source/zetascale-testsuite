    ...
    foreach my $p(@prop){
        for(1 .. $ncntr)
        {
            ...
            foreach my $d(@data){
                ...
                #enum updated objs
                $ret = WriteLogObjects($node->conn(0), $cguid, $$d[0], $$d[1], $$d[2], $$d[3]+10, $$d[4]+10, $$d[5]/2, "ZS_WRITE_MUST_EXIST");
                like($ret, qr/OK.*/, $ret);
        ...
        for(1 .. $ncntr)
        {
            ...
            foreach my $d(@data){
                ...
                $ret = ReadLogObjects($node->conn(0), $cguid, $$d[0], $$d[1], $$d[2], $$d[3]+10, $$d[4]+10, $$d[5]/2);
                like($ret, qr/OK.*/, $ret);
            }
            $ret = CloseContainer($node->conn(0), $cguid);
            like($ret,qr/OK.*/, $ret);
