...
sub worker_update{
    ...
    my $ct=50;
    for(1..$nobject){
        my $ret = WriteLogObjects($node->conn($index), $cguid, $ct, "abc", "efg", $valoff+50, 320, 1, "ZS_WRITE_MUST_EXIST");
        like ($ret, qr/^OK.*/, $node->{'port'}.":".$ret);
...
sub worker_read{
    ...
    my $ct=50;
    for(1..$count[$index]){
        my $ret = ReadLogObjects($node->conn($index), $cguid, $ct, "abc", "efg", $valoff+50, 320, 1);
        like($ret, qr/^OK.*/, $ret);
        $ct++;
    }
}
...
sub test_run {
    ...
    foreach my $p(@prop){
        ...
        foreach(1..$ncntr){
            my $cguid = $cguids[$_-1];
            print "start to update, cguid=$cguid\n";
            push(@threads, threads->new(\&worker_update,$cguid,$_));
        }
        sleep(30);
