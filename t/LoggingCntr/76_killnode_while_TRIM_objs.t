...
sub worker_trim{
    ...
    my $ct=50;
    for(1..$nobject){
        my $ret = WriteLogObjects($node->conn($index), $cguid, $ct, "abc", "efg", $valoff, 160, 1, "ZS_WRITE_TRIM");
        like ($ret, qr/^OK.*/, $node->{'port'}.":".$ret);
...
sub worker_read{
    my ($cguid,$index) = @_;
    my $ct=50+$count[$index];
    for($count[$index]+1..$nobject){
        my $ret = ReadLogObjects($node->conn($index), $cguid, $ct, "abc", "efg", $valoff, 160, 1);
        like($ret, qr/^OK.*/, $ret);
        $ct++;
    }
...
sub test_run {
    ...
    foreach my $p(@prop){
        ...
        foreach(1..$ncntr){
            my $cguid = $cguids[$_-1];
            print "start to trim, cguid=$cguid\n";
            push(@threads, threads->new(\&worker_trim,$cguid,$_));
        }
        sleep(30);
        ...
        foreach(1..$ncntr){
            my $cguid = $cguids[$_-1];
            push(@threads, threads->new(\&worker_read_failed,$cguid,$_));
        }
        $_->join for (@threads);
