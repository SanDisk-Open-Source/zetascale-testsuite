...
sub worker{
    ...
    for (@$cguids_ref){
        foreach my $d(@data){
            $ret = DeleteLogObjects($node->conn($connid), $_, $$d[0]+$nops*$$d[4]*$connid, $$d[1], $$d[2], $nops*$$d[4]);
            like($ret, qr/.*/, $ret);
        }
        $i = $i + 1;
    }
    ...
    foreach my $p(@prop){
        ...
        for my $i(0 .. $ncntr - 1){
            for my $j(0 .. $nthread - 1){
                push(@threads, threads->new(\&read, $node->conn($i*$nthread+$j),$j,$cguids[$i],$valoff,0,$up_per_th,$flags[$j]));
            }
        }
        $_->join for (@threads);
