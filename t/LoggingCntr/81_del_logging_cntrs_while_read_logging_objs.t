...
sub worker{
    ...
    for (@$cguids_ref){
        foreach my $d(@data){
            $ret = ReadLogObjects($node->conn($connid), $_, $$d[0]+$nops*$$d[4]*$connid, $$d[1], $$d[2], $valoff+$nops*$$d[4]*$connid, $$d[3], $nops*$$d[4]);
            like($ret, qr/.*/, $ret);
        }
    }
}
    ...
    foreach my $p(@prop){
        ...
        for my $i(1 .. $ncntr - 1){
            for my $j(0 .. $nthread - 1){
                push(@threads, threads->new(\&read, $node->conn($i*$nthread+$j),$j,$cguids[$i],$valoff,$nops_per_th,1));
            }
        }
        $_->join for (@threads);
        ...
        @threads = ();
        for my $j(0 .. $nthread - 1){
            push(@threads, threads->new(\&read, $node->conn($j),$j,$cguids[0],$valoff,$nops_per_th,0));
        }
        $_->join for (@threads);
