...
sub worker{
    my ($conn, $cguid0, $cguid, $valoff, $nops, $mode) = @_;
    my $ret;
    ...
    foreach my $d(@data){
        $ret = WriteLogObjects($conn, $cguid, $$d[0], $$d[1], $$d[2], $valoff, $$d[3], $nops*$$d[4], "ZS_WRITE_MUST_EXIST");
        like($ret, qr/^OK.*/, $ret);
    }
...
sub test_run {
    ...
    foreach my $p(@prop){
        ...
        my $other_per_th = ($nobject-$update)/$nthread;
        @threads = ();
        for my $i(0 .. $ncntr - 1){
            for my $j(0 .. $nthread - 1){
