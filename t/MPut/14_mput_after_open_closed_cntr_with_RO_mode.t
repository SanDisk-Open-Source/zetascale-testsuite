...
sub test_run {
    ...
    #my @prop = (["yes","yes","no","ZS_DURABILITY_HW_CRASH_SAFE","no"],["yes","yes","no","ZS_DURABILITY_HW_CRASH_SAFE","no"],);
	...
	$cguid = $1 if ($ret =~ /OK cguid=(\d+)/);
	like ($ret, qr/OK.*/, "ZSopenContainer: $cname, cguid=$cguid flags=RO_MODE");
	...
	foreach my $d(@data){
			...
			flags       => "ZS_WRITE_MUST_NOT_EXIST",
                ...
                like ($ret, qr/ERROR.*/, "ZSMPut: can't load objects after open cntr with read_only mode, $ret");	
        }
    	$ret = ZSCloseContainer ($node->conn (0), cguid => "$cguid",);
    	like ($ret, qr/OK.*/, "ZSCloseContainer: cguid=$cguid");
