...
sub test_run {
    ...
    #my $data_len = $val_len + 10;
    my $data_offset_add = $val_offset + 10;
    for (1 .. $ncntr) {
    foreach my $d(@data){
        $ret = ZSMPut (
        $node->conn (0),
        ...
        like ($ret, qr/OK.*/,
            "ZSMPut: load $$d[2] objects keylen= $$d[1] datalen=$$d[2] data_offset=$val_offset to $cname$_, cguid=$cguids[$_-1]");
		...
		key_offset  => $key_offset,
		key_len     => $$d[0],
		data_offset => $data_offset_add,
		data_len    => $$d[1],
		num_objs    => $$d[2],
		flags       => "ZS_WRITE_MUST_EXIST",
        );
        like ($ret, qr/OK.*/,
            "ZSMPut: update $$d[2] objects keylen= $$d[1] datalen=$$d[2] data_offset=$data_offset_add to $cname$_, cguid=$cguids[$_-1]");
