...
sub worker{
    my ($conn, $cguid_ref, $key_offset, $key_len, $val_offset, $val_len, $nops) = @_;
    my ($ret);
    ...
    like($ret, qr/OK.*/, "ZSTransactionSetMode success");

    $ret = ZSTransactionGetMode(
        $conn,
    );
    like($ret, qr/OK.*/, "ZSTransactionGetMode success:".$ret);   
    ...
    for (@$cguid_ref){
        ...
        like($ret, qr/OK.*/, "ZSWriteObject, update $nops objects on cguid=$_");
    }
